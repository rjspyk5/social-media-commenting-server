...
export const getAllComments = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const result = await getAllCommentService({ page: Number(page), limit: Number(limit), sort: sort as string, })
...
export const deleteComment = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userId = req?.user?.id
    try {
        const result = await delteCommentService(commentId as string, userId)
